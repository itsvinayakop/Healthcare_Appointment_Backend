...
export class BookingService {
    ...
    async createProfile(userId: string, userRole: UserRole, dto: CreateDoctorDto): Promise<DoctorProfile> {
        if (userRole !== UserRole.DOCTOR) {
            throw new ForbiddenException('Only users with the DOCTOR role can create a profile.');
        }

        let profile = await this.doctorProfileRepository.findOneBy({ user_id: userId });
    ...
    async createSlots(doctorUserId: string, dto: CreateSlotDto): Promise<AvailabilitySlot[]> {
        let start = new Date(dto.startTime);
        const end = new Date(dto.endTime);
        const slots: AvailabilitySlot[] = [];
        const CONSULTATION_DURATION_MS = 30 * 60 * 1000;

        const profile = await this.doctorProfileRepository.findOneBy({ user_id: doctorUserId });
        if (!profile) {
            throw new NotFoundException('Doctor profile not found. Please create profile first.');
        }
        ...
        if (slots.length === 0) {
            throw new BadRequestException('No valid slots could be created in the given time range.');
        }
    ...
    async findAvailableSlots(specialty: string, date: string): Promise<AvailabilitySlot[]> {
        const cacheKey = `slots:${specialty}:${date}`;
        
        const cachedSlots = await this.cacheManager.get<AvailabilitySlot[]>(cacheKey);
        if (cachedSlots) {
        ...
        const availableSlots = await this.slotRepository.createQueryBuilder('slot')
    ...
    async createBooking(patientId: string, dto: CreateBookingDto): Promise<Booking> {
        const { slotId } = dto;
        ...
        return this.slotRepository.manager.transaction(async (entityManager) => {
            ...
            const doctorProfile = await entityManager.findOneBy(DoctorProfile, { user_id: slot.doctor_id });
